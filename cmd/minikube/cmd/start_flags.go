...
func validateVfkitNetwork(n string) string {
	if runtime.GOOS != "darwin" {
		exit.Message(reason.Usage, "The vfkit driver network is only supported on macOS")
	}
	switch n {
	case "nat":
		// always available
...
func generateNewConfigFromFlags(cmd *cobra.Command, k8sVersion string, rtime string, drvName string) config.ClusterConfig {
	...
	if !(driver.IsKIC(drvName) || driver.IsKVM(drvName) || (driver.IsQEMU(drvName) || driver.IsVFKit(drvName)) && viper.GetString(network) != "") {
